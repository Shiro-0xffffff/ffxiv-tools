 ...
 * CSV 数据文件处理
 */
import { type GameData, type GameDataField, type GameDataRecord, type GameDataType } from './game-data'

/**
 * CSV 数据文件路径
 */
const csvDataFilePath = (version: string, path: string): string => (
...
async function * readCsvDataFile (version: string, path: string): AsyncIterableIterator<string> {
  ...
  if (res.body === null) throw new Error('read data failed')
...
function splitStringWithQuotesPreserved (string: string, separator: string): string[] {
  return string.split(separator).reduce<string[]>((segments, segment) => {
...
function parseCsvCellContent (content: string): string | number | boolean | null {
  ...
  // 字符串类型，需要对转义过的字符进行解码
  if (content.match(/^"(.|\s)*"$/) === null) return content.slice(1, -1).replace(/\\./g, char => ({ '\\n': '\n' }[char] ?? char[1]))
  ...
  const records = (async function * (): AsyncIterableIterator<GameDataRecord<T>> {
    for await (const dataRowContent of lines) {
      ...
      fieldsData.forEach((fieldData, index) => {
        const key = fields[index].key !== '' ? fields[index].key : `$${index}` as keyof T
        data[key] = fieldData
      })
